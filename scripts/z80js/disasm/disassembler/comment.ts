...
export class Comment {
  ...
  public addBefore(line: string) {
    if (!this.linesBefore) this.linesBefore = Array<string>();
    this.linesBefore.push(line);
  }

  /// Adds a line to the 'after'-comment.
  public addAfter(line: string) {
    if (!this.linesAfter) this.linesAfter = Array<string>();
  ...
  public static getLines(
    ...
    if (!disableComments && comment) {
      ...
      if (comment.linesBefore) arr.push(...comment.linesBefore);
      let text = statement;
      if (comment.inlineComment) text += "\t" + comment.inlineComment;
      arr.push(text);
      if (comment.linesAfter) arr.push(...comment.linesAfter);
      return arr;
    } else {
      // no comment
      return [statement];
