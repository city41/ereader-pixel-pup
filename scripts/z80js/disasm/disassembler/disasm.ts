...
export class Disassembler extends EventEmitter {
  ...
  protected statisticsMax: SubroutineStatistics = {
    sizeInBytes: 0,
    countOfInstructions: 0,
    CyclomaticComplexity: 0,
  };

  /// The statistics minimum
  protected statisticsMin: SubroutineStatistics = {
  ...
  public rstDontFollowAddresses = [0, 8]; // new Array<number>();
  ...
  constructor() {
    ...
    Opcode.setConvertToLabelHandler((value) => {
      ...
      let label;
      let offsString = "";
      if (this.labels) label = this.labels.get(value);
      if (!label) {
        // Check for offset label
        const offs = this.offsetLabels.get(value);
        if (offs) {
          label = this.labels.get(value + offs);
          if (label) offsString = offs > 0 ? "" + -offs : "+" + -offs;
        }
      }
      if (label) valueName = label.name + offsString;
      return valueName;
    });
  }
  ...
  public addAutomaticAddresses() {
    if (!this.automaticAddresses) return;
  ...
  public disassemble() {
    ...
    // Add label comments
    if (!this.disableCommentsInDisassembly) this.addLabelComments();
    ...
    while (this.disassembledLines.length) {
      if (this.disassembledLines[0].length > 0) break;
  ...
  private isInDataRegion(address: number): boolean {
    return this.dataRegions.some((dr) => {
      return address >= dr.start && address < dr.start + dr.size;
    });
  }
  ...
  public setLabel(address: number, name?: string, type = NumberType.CODE_LBL) {
    if (this.isInDataRegion(address)) {
      return;
    }
    ...
    if (label) {
      ...
      if (!label.name || !label.isFixed)
        // Don't overwrite names given for fixed addresses.
  ...
  public setFixedCodeLabel(address: number, name?: string) {
    ...
    // Set name
    if (name) (label.name as any) = name;
    // Check if out of range
    const attr = this.memory.getAttributeAt(address);
    if (attr & MemAttribute.ASSIGNED) this.addressQueue.push(address);
    else label.isEqu = true; // out of range -> EQU
    // Set as fixed
    label.isFixed = true;
  }
  ...
  public useMameTraceFile(path: string) {
    ...
    for (let addr = 0; addr < MAX_MEM_SIZE; addr++) {
      if (buffer[addr]) this.addressQueue.push(addr);
    }
  }
    ...
    for (let [address, label] of this.labels) {
      ...
      if (label.isEqu) {
        ...
        if (!label) {
          ...
          console.log("label.name is not defined");
        }
        // "Disassemble"
        const statement =
          Format.addSpaces(label.name, this.clmnsBytes - 1) +
          " " +
          this.rightCase("= ") +
          Format.fillDigits(Format.getHexString(address, 4, "0x"), " ", 5);
        // Comment
        const comment = this.addressComments.get(address);
        const commentLines = Comment.getLines(
          comment,
          statement,
  ...
  protected collectLabels() {
    ...
    while ((address = this.addressQueue.shift()) != undefined) {
      ...
      assert(
        ...
        `collectLabels: address ${address} out of range ${this.memory.size}`
      );
      ...
      do {
        ...
        if (attr & MemAttribute.CODE) break; // Yes, already disassembled
        if (!(attr & MemAttribute.ASSIGNED)) {
          // Error: trying to disassemble unassigned memory areas
          if (!this.no_warning_disassemble_unassigned_memory)
            this.emit(
              "warning",
              "Trying to disassemble unassigned memory area at 0x" +
                address.toString(16) +
                "."
            );
          break;
        ...
        if (this.DBG_COLLECT_LABELS)
          console.log(
            Format.getHexString(address) + "\t" + opcode.disassemble().mnemonic
          );
        ...
        for (let i = 1; i < len; i++) {
          memAddress++;
          assert(
            memAddress < this.memory.size,
            `collectLabels: memAddress ${memAddress} out of range ${this.memory.size}`
          );
          ...
          if (attr & MemAttribute.CODE) {
            ...
            this.emit(
              "warning",
              'Aborting disassembly: Ambiguous disassembly: Trying to disassemble opcode "' +
                opcode.name +
                '" at address 0x' +
                address.toString(16) +
                " but address 0x" +
                memAddress.toString(16) +
                ' already contains opcode "' +
                otherOpcode.name +
                '".'
            );
        ...
        if (opcode.flags & OpcodeFlag.STOP) break;
      ...
      if (this.DBG_COLLECT_LABELS) console.log("\n");
    }
  }
   ...
   */
  protected setFoundLabel(
    address: number,
    referenceAddresses: Set<number>,
    type: NumberType,
    attr: MemAttribute
  ) {
    if (this.isInDataRegion(address)) {
    ...
    if (label) {
      ...
      if (label.type < type) label.type = type;
    } else {
      // Label does not exist yet, just add it
      label = new DisLabel(type);
      this.labels.set(address, label);
      // Check if out of range
      if (!(attr & MemAttribute.ASSIGNED)) label.isEqu = true;
    }
  ...
  protected setSpecialLabels() {
    if (this.automaticAddresses) {
      ...
      if (this.snaStartAddress >= 0) {
        ...
        if (!label)
          // if not found by other means.
          this.setLabel(
            this.snaStartAddress,
            "SNA_LBL_MAIN_START_" +
              this.snaStartAddress.toString(16).toUpperCase(),
            NumberType.CODE_LBL
          );
      }
    }
      ...
      if ((prevAttr ^ memAttr) & MemAttribute.ASSIGNED) {
        ...
        if (memAttr & MemAttribute.ASSIGNED) {
          ...
          if (!label) {
            this.setLabel(
              addr,
              "BIN_START_" + Format.getHexString(addr, 4, "").toUpperCase(),
              NumberType.DATA_LBL
            );
          }
        }
  ...
  protected findInterruptLabels() {
    ...
    for (let address = 0x0000; address < MAX_MEM_SIZE; address++) {
      ...
      if (address != this.snaStartAddress) {
        if (
          memAttr & MemAttribute.CODE_FIRST &&
          ...
          if (!label) {
            ...
            if (
              !(prevAttr & MemAttribute.ASSIGNED) ||
      ...
      // Backup values
      prevAttr = memAttr;
      if (!(memAttr & MemAttribute.CODE)) prevCodeAddr = -1;
      if (memAttr & MemAttribute.CODE_FIRST) prevCodeAddr = address;
    }
  ...
  protected disassembleForLabel(
    ...
    if (opcode.flags & OpcodeFlag.BRANCH_ADDRESS) {
      ...
      if (vType == NumberType.CODE_LOCAL_LBL) {
        ...
        if (branchAddress <= opcodeAddress) vType = NumberType.CODE_LOCAL_LOOP;
      } else if (vType == NumberType.CODE_LBL) {
        // Treat JP to unassigned memory area as a CALL.
        if (!(attr & MemAttribute.ASSIGNED)) vType = NumberType.CODE_SUB;
      }
      ...
      if (!(attr & MemAttribute.CODE)) {
        ...
        if (attr & MemAttribute.ASSIGNED) {
          ...
          if (
            vType != NumberType.CODE_RST ||
            this.rstDontFollowAddresses.indexOf(branchAddress) < 0
          ) {
  ...
  protected adjustCodePointingLabels() {
    ...
    for (let [address, label] of changeMap) {
      ...
      do {
        ...
        assert(
          address - addrStart <= 4,
          `adjustSelfModifyingLabels: ${JSON.stringify(
            label
          )}, ${address.toString(16)}`
        ); // Opcode should be smaller than 5 bytes
   ...
   */
  protected findNextFlowThroughLabel(
    address: number
  ): { label: DisLabel; address: number } | undefined {
      ...
      if (label.type == NumberType.CODE_LBL) {
        ...
        if (retFound) {
          ...
          DelayedLog.logIf(
            ...
            () =>
              "Addresses: " +
              addrsArray.map((addr) => addr.toString(16)).join(" ")
          );
          DelayedLog.logIf(
            address,
            () =>
              "turnLBLintoSUB: Turned Label " +
              getNumberTypeAsString(label.type) +
              " into CODE_SUB."
          );
    ...
    do {
      ...
      if (!(memAttr & MemAttribute.ASSIGNED)) {
        DelayedLog.log(
          () =>
            "findRET: address=" +
            DelayedLog.getNumber(address) +
            ": returns. memory not assigned."
        );
        return false;
      }
      ...
      if (addrsArray.indexOf(address) >= 0) {
        DelayedLog.log(
          () =>
            "findRET: address=" +
            DelayedLog.getNumber(address) +
            ": returns. memory already checked."
        );
      ...
      if (addrLabel) {
        ...
        if (type == NumberType.CODE_SUB || type == NumberType.CODE_RST) {
          DelayedLog.log(
            () =>
              "findRET: address=" +
              DelayedLog.getNumber(address) +
              ": SUB FOUND. address belongs already to a SUB."
          );
          return true;
        }
      }
        ...
        if (!(opcodeClone.flags & OpcodeFlag.CALL)) {
          ...
          DelayedLog.log(
            () =>
              ...
              DelayedLog.getNumber(address) +
              ": branching to " +
              DelayedLog.getNumber(branchAddress) +
              "."
          );
          DelayedLog.pushTab();
          const res = this.findRET(branchAddress, addrsArray);
          DelayedLog.popTab();
          if (res) return true; // SUB found
        }
      }
      ...
      switch (label.type) {
        ...
        case NumberType.CODE_RST:
          ...
          for (let addr of addrsArray) {
            ...
            if (
              addrLabel.type != NumberType.CODE_LBL &&
              addrLabel.type != NumberType.CODE_SUB
            )
              continue;
            if (addrLabel.isFixed) continue;
            ...
            if (!outsideFound) {
              ...
              DelayedLog.logIf(
                addr,
                () =>
                  "findLocalLabelsInSubroutines: Turned Label" +
                  getNumberTypeAsString(addrLabel.type) +
                  " into CODE_LOCAL_LBL."
              );
  ...
  protected getSubroutineAddresses(address: number, addrsArray: Array<number>) {
    ...
    do {
      ...
      if (!(memAttr & MemAttribute.ASSIGNED)) {
        DelayedLog.log(
          () =>
            "getSubroutineAddresses: address=" +
            DelayedLog.getNumber(address) +
            ": returns. memory not assigned."
        );
        break;
   ...
   */
  protected reduceSubroutineAddresses(
    address: number,
    addrsArray: Array<number>
  ) {
    // sort array
    ...
    for (let [address, label] of this.labels) {
      ...
      if (
        ...
        this.emit(
          "warning",
          "Address: " +
            Format.getHexString(address, 4) +
            "h. A subroutine was found that calls itself recursively but is not called from any other location."
        );
      ...
      switch (label.type) {
        ...
        case NumberType.CODE_LBL:
          ...
          if (
            ...
            this.statisticsMax.countOfInstructions
          )
            this.statisticsMax.countOfInstructions =
              statistics.countOfInstructions;
          if (
            statistics.CyclomaticComplexity >
            this.statisticsMax.CyclomaticComplexity
          )
            this.statisticsMax.CyclomaticComplexity =
              statistics.CyclomaticComplexity;
          // Get min
          ...
          if (
            statistics.countOfInstructions <
            this.statisticsMin.countOfInstructions
          )
            this.statisticsMin.countOfInstructions =
              statistics.countOfInstructions;
          if (
            statistics.CyclomaticComplexity <
            this.statisticsMin.CyclomaticComplexity
          )
            this.statisticsMin.CyclomaticComplexity =
              statistics.CyclomaticComplexity;
      }
   ...
   */
  protected countAddressStatistic(
    address: number,
    addresses: Array<number>
  ): SubroutineStatistics {
    let statistics = {
      sizeInBytes: 0,
      countOfInstructions: 0,
      CyclomaticComplexity: 0,
    ...
    do {
      ...
      // Unfortunately it needs to be checked if address has been checked already
      if (addresses.indexOf(address) >= 0) return statistics; // already checked
      // Add to array
      ...
      if (opcodeClone.flags & OpcodeFlag.BRANCH_ADDRESS) {
        ...
        if (opcode.name.indexOf(",") >= 0) statistics.CyclomaticComplexity++;
      } else if (
        opcodeClone.flags & OpcodeFlag.RET &&
      ...
      if (opcodeClone.flags & OpcodeFlag.BRANCH_ADDRESS) {
        if (!(opcodeClone.flags & OpcodeFlag.CALL)) {
          ...
          if (!isSUB) {
            const addStat = this.countAddressStatistic(
              branchAddress,
              addresses
            );
      ...
      if (nextLabel) {
        ...
        if (type == NumberType.CODE_SUB || type == NumberType.CODE_RST) break; // Stop when entering another subroutine.
      }
    } while (!(opcodeClone.flags & OpcodeFlag.STOP));
  ...
  protected assignLabelNames() {
      ...
      switch (type) {
        ...
        case NumberType.CODE_LOCAL_LOOP:
          ...
          if (parentLabel) {
            ...
            const arr =
              type == NumberType.CODE_LOCAL_LBL ? localLabels : localLoops;
    ...
    const labelSelfModifyingCountDigits =
      labelSelfModifyingCount.toString().length;
    ...
    for (const [address, label] of this.labels) {
      ...
      switch (type) {
        case NumberType.CODE_SUB:
          // Set name
          label.name = label.belongsToInterrupt
            ? this.labelIntrptPrefix
            : "" +
              this.labelSubPrefix +
              this.getIndex(subIndex, labelSubCountDigits);
          // Next
          subIndex++;
          break;
        case NumberType.CODE_LBL:
          ...
          label.name = label.belongsToInterrupt
            ...
            : "" +
              this.labelLblPrefix +
              this.getIndex(lblIndex, labelLblCountDigits);
          // Next
          lblIndex++;
          break;
        case NumberType.CODE_RST:
          // Set name
          label.name =
            this.labelRstPrefix + Format.getHexString(address, 2, "");
          break;
        case NumberType.DATA_LBL:
          ...
          if (memAttr & MemAttribute.CODE) {
            ...
            // Set name
            label.name =
              this.labelDataLblPrefix +
              this.getIndex(dataLblIndex, labelDataLblCountDigits);
            // Next
            dataLblIndex++;
          }
          break;
    ...
    for (const [parentLabel, childLabels] of localLabels) {
      ...
      for (let child of childLabels) {
        ...
        if (count > 1) child.name += indexString;
        index++;
      }
    }
    // Local Loops:
  ...
  protected addLabelComments() {
    ...
    for (const [address, label] of this.labels) {
      ...
      if (comment) this.addressComments.set(address, comment);
    }
  }

  protected assignDataRegions() {
    for (const dataRegion of this.dataRegions) {
      this.memory.setAttributesAt(
        dataRegion.start,
        dataRegion.size,
  ...
  protected getAddressComment(address: number): Comment | undefined {
    if (this.disableCommentsInDisassembly) return undefined;
  ...
  protected getLabelComments(address: number): Comment | undefined {
    ...
    switch (type) {
      ...
      case NumberType.CODE_RST: {
        ...
        for (const ref of addrLabel.references) {
          if (!first) line2 += ", ";
          const s = Format.getHexString(ref, 4);
          const parent = this.addressParents[ref];
          if (parent) {
            let parName;
            if (parent == addrLabel) {
        ...
        if (!addrLabel.isEqu) {
          ...
          line3 += Array.from(callees)
            .map((refLabel) => refLabel.name)
            .join(", ");
          // Check if anything has been output
      ...
      default: {
        ...
        if (refCount > 0) {
          ...
          const refArray = [...addrLabel.references].map((addr) => {
            let s = Format.getHexString(addr, 4);
            const parentLabel = this.addressParents[addr];
            if (parentLabel) {
    ...
    if (addrLabel.isEqu) {
      comment.inlineComment =
        "; " + address.toString() + ". " + lineArray.join(" ");
    } else {
    ...
    for (const line of commentsLines) {
      ...
      switch (state) {
        ...
        case State.LinesAfter:
          if (addressPart) {
            throw Error(
              'Expected "after"-comment or newline: "' +
                line +
                '", ' +
                commentsFile +
                ":" +
  ...
  protected disassembleMemory(): Array<string> {
    ...
    if (this.labels.size == 0) return lines;

    // Loop over all labels
    let address = -1;
    for (const [addr, label] of this.labels) {
      if (label.isEqu) continue; // Skip EQUs

      // If first line, print AREA and ORG
      if (address == -1) {
        // First line, set to code area
        this.addEmptyLines(lines);
        const areaLine =
          " ".repeat(this.clmnsBytes) + this.rightCase(".area ") + "CODE (ABS)";
        lines.push(areaLine);

        // Second line
        // Print "ORG"
        let orgLine =
          " ".repeat(this.clmnsBytes) +
          this.rightCase(".ORG ") +
          Format.getHexString(addr, 4, "0x");
        if (!this.disableCommentsInDisassembly)
      ...
      while (true) {
        ...
        if (addrLabel) {
          ...
          if (this.clmnsAddress > 0) {
            labelLine =
              Format.addSpaces(
                Format.getHexString(address),
                this.clmnsAddress
              ) + labelLine;
            if (this.DBG_ADD_DEC_ADDRESS) {
              labelLine =
                Format.addSpaces(address.toString(), 5) + " " + labelLine;
            }
          }
        ...
        if (attr & MemAttribute.CODE) {
          ...
          line = this.formatDisassembly(
            address,
            opcode.length,
            opCodeDescription.mnemonic
          );
          // only show erapi api comments
          commentText = opcode.appendApi ? opCodeDescription.comment : "";
          ...
          let mainString =
            this.rightCase(".db ") + Format.getHexString(memValue, 2, "0x");
          // only showing erapi api comments
   ...
   */
  protected formatDisassembly(
    address: number,
    size: number,
    mainString: string
  ): string {
    const memory = this.addOpcodeBytes ? this.memory : undefined;
    return Format.formatDisassembly(
      memory,
      this.opcodesLowerCase,
      this.clmnsAddress,
      this.clmnsBytes,
      this.clmnsOpcodeFirstPart,
  ...
  protected addEmptyLines(lines: Array<string>) {
    if (this.emptyLinesWritten) return; // Already written
  ...
  protected rightCase(s: string): string {
    // Lowercase?
    if (this.opcodesLowerCase) return s.toLowerCase();
    return s;
  }
  ...
  public getGraphLabels(
    addrString: number | string,
    chosenLabels?: Map<number, DisLabel>
  ): Map<number, DisLabel> {
    // Crete new map
    if (!chosenLabels) chosenLabels = new Map<number, DisLabel>();
    ...
    if (typeof addrString == "string") {
      ...
      if (!addr)
        throw Error(
          'Could not find "' + addrString + '" while creating graph.'
        );
    } else {
      // Number
      addr = addrString;
    ...
    if (!label)
      throw Error(
        'Could not find address for "' + addrString + '" while creating graph.'
      );
  ...
  public getCallGraph(labels: Map<number, DisLabel>): string {
    ...
    const fontSizeFactor =
      (fontSizeMax - fontSizeMin) / (complDiff < 8 ? 8 : complDiff);
    ...
    for (const [address, label] of labels) {
      ...
      if (label.isEqu) {
        ...
        if (!colorString) colorString = "lightgray";
        text += label.name + ' [fontsize="' + fontSizeMin + '"];\n';
        const nodeName = this.nodeFormat(
          label.name,
          label.id,
          address,
          undefined,
        ...
        assert(stats);
        const fontSize =
          fontSizeMin + fontSizeFactor * (stats.CyclomaticComplexity - min);

        // Output
        text +=
          '"' + label.name + '" [fontsize="' + Math.round(fontSize) + '"];\n';
        const nodeName = this.nodeFormat(
          label.name,
          label.id,
          address,
        ...
        if (label.references.size == 0 || label.type == NumberType.CODE_LBL) {
          ...
          if (!colorString) colorString = "lightyellow";
          if (label.references.size == 0) rankSame1.push(label.name);
          else rankSame2.push(label.name);
        }
  ...
  public getFlowChart(startAddresses: Array<number>): string {
    ...
    for (const startAddress of startAddresses) {
      ...
      if (!name) name += "0x" + addressString;
      const start = "b" + addressString + "start";
      text +=
        start +
        ' [label="' +
        name +
        '", fillcolor=lightgray, style=filled, shape=tab];\n';
      ...
      const bText = this.getBranchForAddress(
        startAddress,
        addrsArray,
        processedAddrsArray
      );
   ...
   */
  protected getBranchForAddress(
    address: number,
    addrsArray: Array<number>,
    processedAddrsArray: Array<number>
  ): string {
    ...
    do {
      ...
      if (this.labels.get(address)) break;
    } while (
      !(opcode.flags & OpcodeFlag.BRANCH_ADDRESS) && // branch-address includes a CALL
    ...
    if (opcode.flags & OpcodeFlag.BRANCH_ADDRESS) {
      ...
      if (addrsArray.indexOf(branchAddress) >= 0) {
        ...
        text +=
          branch +
          " -> b" +
          Format.getHexString(branchAddress, 4) +
          ' [headport="n", tailport="e"];\n';
        // Check if already disassembled
        if (processedAddrsArray.indexOf(branchAddress) < 0) {
          // No, so disassemble
          const textBranch = this.getBranchForAddress(
            branchAddress,
            addrsArray,
            processedAddrsArray
          );
    ...
    if (opcode.flags & (OpcodeFlag.STOP | OpcodeFlag.RET)) {
      ...
      text +=
        branch + " -> b" + Format.getHexString(addrsArray[0], 4) + "end;\n";
    }
