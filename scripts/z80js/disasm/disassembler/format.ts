...
export class Format {
   ...
   */
  public static getHexString(
    value: number,
    countDigits = 4,
    overrideHexStringFormat?: HexStringFormat
  ): string {
    let s = value.toString(16);
    if (!Format.hexNumbersLowerCase) s = s.toUpperCase();
    const bareHexNumber = Format.fillDigits(s, "0", countDigits);

    const hexStringFormat = overrideHexStringFormat ?? Format.hexStringFormat;

    switch (hexStringFormat) {
      case "$":
        return "$" + bareHexNumber;
      case "0x":
        return "0x" + bareHexNumber;
      case "#0x":
        return "#0x" + bareHexNumber;
      case "h":
        return bareHexNumber + "h";
      case "":
        return bareHexNumber;
      default:
        return "#0x" + bareHexNumber;
   ...
   */
  public static fillDigits(
    valueString: string,
    fillCharacter: string,
    countDigits: number
  ): string {
    const repeat = countDigits - valueString.length;
    if (repeat <= 0) return valueString;
  ...
  public static getVariousConversionsForByte(byteValue: number): string {
    // byte
    if (byteValue < 0) byteValue = 0x100 + byteValue;
  ...
  public static getConversionForAddress(value: number): string {
    // word
    let result = Format.getHexString(value, 4, "0x");
    // return
    return result;
  }
   ...
   */
  public static formatDisassembly(
    memory: BaseMemory | undefined,
    opcodesLowerCase: boolean,
    clmnsAddress: number,
    clmnsBytes: number,
    clmnsOpcodeFirstPart: number,
    clmsnOpcodeTotal: number,
    address: number,
    size: number,
    mainString: string
  ): string {
