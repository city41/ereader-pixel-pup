...
class Startup {
  ...
  public static main(): number {
    try {
      ...
      // Lower case opcodes?
      if (this.dasm.opcodesLowerCase) Opcode.makeLowerCase();

      // infile with comments for the labels
      if (this.commentsPath) this.dasm.setAddressComments(this.commentsPath);
      ...
      if (this.callGraphOutPath) {
        ...
        if (chosenLabels.length) {
          ...
          if (k >= 0) mainPath = mainPath.substr(0, k);
          mainPath += "_";
          ...
          for (const addrString of this.dasm.graphLabels) {
            ...
            if (typeof name == "number") {
              ...
              if (label && label.name) name = label.name;
              else name = "0x" + name.toString(16);
            }
...
z80dismblr [options]
    ...
    while ((arg = args.shift())) {
      ...
      switch (arg) {
        ...
        case "--subprefix":
          // get prefix
          text = args.shift();
          if (!text || text.length == 0) throw arg + ": No prefix given";
          // Set prefix
        ...
        case "--clmnsaddress":
          ...
          if (isNaN(value)) throw arg + ": Wrong value " + text;
        ...
        case "--callgraphnode":
          {
            ...
            if (isNaN(addr)) this.dasm.graphLabels.push(node);
            else {
              // Number
        ...
        case "--data-region":
          const dataRegionStartAddressString = args.shift();
          const dataRegionStartAddress = this.parseValue(
            dataRegionStartAddressString
          );
          if (isNaN(dataRegionStartAddress)) {
            throw (
              arg +
              ": data region start address is not-a-number: " +
              dataRegionStartAddressString
            );
          }
          const dataRegionSizeString = args.shift();
          const dataRegionSize = this.parseValue(dataRegionSizeString);
          if (isNaN(dataRegionSize)) {
            throw (
              arg +
              ": data region size is not-a-number: " +
              dataRegionSizeString
            );
          }

          this.dasm.dataRegions.push({
            start: dataRegionStartAddress,
            size: dataRegionSize,
          });
          break;

        case "--missing-memory-value":
          const missingMemoryValueString = args.shift();
          const missingMemoryValue = this.parseValue(missingMemoryValueString);
          if (isNaN(missingMemoryValue)) {
            throw (
              arg +
              ": missing-memoyr-value is not-a-number: " +
              missingMemoryValueString
            );
          }
          this.dasm.memory.missingMemoryValue = missingMemoryValue;
          break;
  ...
  protected static readArgsFromFile(path: string): Array<string> {
    ...
    while (true) {
      ...
      while (true) {
        ...
        if (k >= len) return args; // End of file reached
        // check for comment
        val = argsData[k];
        if (val != "#") break;
        // skip comment
        k = argsData.indexOf("\n", k + 1);
        if (k < 0) return args; // End of file reached
      }
  ...
  protected static skipWhiteSpaces(s: string, i: number): number {
    ...
    while (i < len) {
      const val = s[i];
      if (this.whiteSpaces.indexOf(val) < 0) break;
      // next
      i++;
    }
    return i;
  ...
  public static parseValue(valueString: string | undefined): number {
    ...
    const match =
      /^\s*((0x|\$)([0-9a-f]+)([^0-9a-f]*))?(([0-9a-f]+)h(.*))?(([01]+)b(.*))?(_([szhnpc]+)([^szhnpc])*)?((-?[0-9]+)([^0-9]*))?('([\S ]+)')?/i.exec(
        valueString
      );
    if (!match) return NaN; // Error during parsing
    ...
    // Hex
    if (ghex) {
      if (ghex_empty) return NaN;
      return parseInt(ghex, 16);
    }
    if (ghexh) {
      if (ghexh_empty) return NaN;
      return parseInt(ghexh, 16);
    }

    // Decimal
    if (gdec) {
      if (gdec_empty) return NaN;
      return parseInt(gdec, 10);
    }
    // Bits
    if (gbit) {
      if (gbit_empty) return NaN;
