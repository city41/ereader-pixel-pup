...
sizeInput.addEventListener("change", function (e) {
  resizeGrid(parseInt(e.target.value), grid);
  drawInputGrid(grid, inputCanvas, inputCtx);
  calculate();
});
...
fileInput.addEventListener("change", function (e) {
  if (
    ...
    !e.target.accept.includes(e.target.files[0].type)
  )
    return;
  ...
  fr.addEventListener("load", function () {
    generateGridFromImage(fr.result).then((generatedGrid) => {
      grid = generatedGrid;
      calculate();
    });
  });
