...
class Nonogram {
  constructor(groupsHor, groupsVert) {
    console.log("groupsHor", JSON.stringify(groupsHor, null, 2));
    console.log("groupsVert", JSON.stringify(groupsVert, null, 2));
...
class NonogramGUI {
  ...
  constructor(bindDiv) {
    ...
    this.settingsDiv.appendChild(
      this.createInputElement("Width", "width", this.width)
    );
    this.settingsDiv.appendChild(
      this.createInputElement("Height", "height", this.height)
    );
    this.settingsDiv.appendChild(
      this.createInputElement("Group size", "maxGroup", this.maxGroup)
    );
  ...
  displayTable() {
    ...
    for (var i = 0; i < this.height + this.maxGroup; i++) {
      ...
      for (var j = 0; j < this.width + this.maxGroup; j++) {
        ...
        else if (i < this.maxGroup && j >= this.maxGroup) {
          type = "groupVert";
          x = i;
          y = j - this.maxGroup;
        } else if (i >= this.maxGroup && j < this.maxGroup) {
  ...
  prepareGroups(groups) {
    ...
    for (var x = 0; x < groups.length; x++) {
      ...
      for (var y = 0; y < groups[x].length; y++)
        if (groups[x][y] > 0) row.push(groups[x][y]);
      ret.push(row);
    }
    return ret;
  }
  ...
  solve() {
    var nonogram = new Nonogram(
      this.prepareGroups(this.groupsHor),
      this.prepareGroups(this.groupsVert)
    );
